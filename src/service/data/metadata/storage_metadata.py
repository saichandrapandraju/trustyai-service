...
class StorageMetadata:
    ...
    def __init__(self, model_id: str = None, input_schema: Schema = None, output_schema: Schema = None, input_tensor_name: str = "input", output_tensor_name: str = "output", observations: int = 0, recorded_inferences: bool = False):
    ...
    def increment_observations(self, observations: int):
    ...
    def merge_input_schema(self, other_schema: Schema):
        """Merge another schema with the input schema."""
        if other_schema != self.input_schema:
            raise ValueError("Original schema and schema-to-merge are not compatible")
    
    def merge_output_schema(self, other_schema: Schema):
