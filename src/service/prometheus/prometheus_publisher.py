...
class PrometheusPublisher:
    def __init__(self, registry: CollectorRegistry = REGISTRY):
        ...
        # This is because prometheus_client doesn't allow multiple
        # gauges with the same name and 
        # it doesn't expose public methods to retrieve them with name
    ...
    def _set_value(self, id: uuid.UUID, value: float):
        with self._values_lock:
            self.values[id] = value
    
    def _remove_value(self, id: uuid.UUID):
        with self._values_lock:
        ...
        if request is not None:
            ...
            if value is not None:
                ...
                for idx, (key, val) in enumerate(named_values.items()):
                    ...
                    new_id = self._generate_uuid_from_bytes(concat_string)
